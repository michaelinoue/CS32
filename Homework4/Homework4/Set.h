...
template <typename ItemType> class Set
{
public:
...
template <class Set> void unite(const Set& s1, const Set& s2, Set& result);
// result = { x | (x in s1) OR (x in s2) }

template <class Set> void subtract(const Set& s1, const Set& s2, Set& result);
...
template <typename ItemType> Set<ItemType>::Set()
{
	createEmpty();
}

template <typename ItemType> bool Set<ItemType>::insert(const ItemType& value)
{
...
template <typename ItemType> bool Set<ItemType>::erase(const ItemType& value)
{
...
template <typename ItemType> bool Set<ItemType>::get(int i, ItemType& value) const
{
	if (i < 0 || i >= m_size)
		return false;
...
template <typename ItemType> void Set<ItemType>::swap(Set& other)
{
	// Swap head pointers
...
template <typename ItemType> Set<ItemType>::Set(const Set& other)
{
	createEmpty();
...
template <typename ItemType> Set<ItemType>& Set<ItemType>::operator=(const Set& rhs)
{
	if (this != &rhs)
	{
		// Copy and swap idiom
...
template <typename ItemType> void Set<ItemType>::createEmpty()
{
	m_size = 0;
...
template <typename ItemType> void Set<ItemType>::insertBefore(Node* p, const ItemType& value)
{
	// Create a new node
...
template <typename ItemType> void Set<ItemType>::doErase(Node* p)
{
...
template <typename ItemType> typename Set<ItemType>::Node* Set<ItemType>::findFirstAtLeast(const ItemType& value) const
{
...
template <typename ItemType> void subtract(const Set<ItemType>& s1, const Set<ItemType>& s2, Set<ItemType>& result)
{
