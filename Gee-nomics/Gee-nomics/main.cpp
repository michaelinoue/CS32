...
int main()
{
	...
	//string	filename = "D:/data/Ferroplasma_acidarmanus.txt";
	...
	Genome genome("genome1", "AGTAGTAGTAGTAGT");
	Genome genome3("genome3", "AGAGTTTTTTTTTTTTT");
	Genome genome2("genome2", "AGUAGT");
	Genome genome4("genome4", "UGAGU");
	Genome genome5("genome5", "AUTAGTAGTAGT");
	...
	Trie <int> trie;
	trie.insert("AGT", 3);
	vector<int> vec = trie.find("AGU", true);
	cout << vec.size();
	GenomeMatcher gm(3);
	gm.addGenome(genome);
	gm.addGenome(genome3);
	gm.addGenome(genome2);
	gm.addGenome(genome4);
	vector<DNAMatch> dna;
	vector <GenomeMatch> genomeMatch;

	int i = gm.findGenomesWithThisDNA("AGTAGTAGTAGTAGT", 1, true, dna);
	cout << i << endl;
	gm.findRelatedGenomes(genome5, 3, false, 10, genomeMatch);
	for (int i = 0; i < genomeMatch.size(); i++)
	{
		cout << genomeMatch[i].genomeName << "   pm:  " << genomeMatch[i].percentMatch << endl;
	}

	for (int i = 0; i < dna.size(); i++)
	{
		cout << dna[i].genomeName << "    pos: " << dna[i].position <<endl << "    " << dna[i].length;
	} 
...
const string PROVIDED_DIR = "D:/data";
...
void findGenome(GenomeMatcher* library, bool exactMatch)
{
	...
	cout << library->findGenomesWithThisDNA(sequence, minMatchLength, exactMatch, matches);
