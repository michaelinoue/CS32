// Return the number of ways that all n2 elements of a2 appear
	  ...
	  //	20 10 40			0
	  //	50 40 30			3
int countIncludes(const double a1[], int n1, const double a2[], int n2) //change
{
	if (n2 < 0)
		n2 = 0;
	if (n1 < 0)
	...
	if (a2[0] == a1[0])
	{
		int first = countIncludes(a1 + 1, n1 - 1, a2 + 1, n2 - 1);
		int rest = countIncludes(a1 + 1, n1 - 1, a2, n2);
		return first + rest;
	}
	else
...
void order(double a[], int n)
{
	if (n <= 1)
		return;
	int first, less;
	divide(a, n, a[0], first, less);
	if (less == n) //this means that the last element is at the back of the list; we should ignore it and try the others now
		order(a, n - 1);
	else
	{
		order(a, less); //start ordering at the first instance of lesser element; this will get pushed to the back as well
		order(a + less, n - less);
	}
}
